...
df = ld.separateTargetClass(df, "moca_pre_init")

# initial scaling. just to avoid NAN in TS feature extraction
df = sc.useMinMax(df, ['gsId', 'gsTime'])
...
'''
dfAllScaled = sc.useMinMax(dfComplete, ['gsId', 'gsTime'])
...
'''
model = trn.trainLinearRegressionModel(dfVarianceFiltered, ['r2', 'neg_mean_absolute_error'])
